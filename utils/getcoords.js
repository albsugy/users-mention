// @ts-ignore
const GetCoords = (textArea, iterable = false) => {
    let replica = document.createElement("div");
    const copyStyle = getComputedStyle(textArea);

    const iterableCopyStyle = Object.entries(copyStyle);
    const styleProps = iterable ? iterableCopyStyle : copyStyle;
    ...
    return {
      x: (spanWidth > textArea.offsetWidth ? textArea.offsetWidth : spanWidth) + textArea.offsetLeft + 12,
      y: (spanHeight > textArea.offsetHeight ? textArea.offsetHeight: spanHeight) + textArea.offsetTop + 12
    };
  };
